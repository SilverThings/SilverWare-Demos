...
public class WorkflowRoutes extends RouteBuilder {
   ...
   public void configure() throws Exception {
      ...
      from("mqtt:inCommands?subscribeTopicName=ih/message/commands&userName=mqtt&password=mqtt&host=tcp://" + mqttHost)
            .choice()
      ...
      from("direct:led").choice()
            .when().simple("${body.place} == io.silverware.demos.devconf.kjar.LightCommand.Place.ALL").to("direct:ledAll")
      ...
      from("direct:ledAll").setHeader(Exchange.HTTP_METHOD, constant("GET"))
            .setHeader("r", simple("${body.state.r}")).setHeader("g", simple("${body.state.g}")).setHeader("b", simple("${body.state.b}"))
            .to("jetty:http://" + iotHost + "/led/setrgb/all");
      ...
      from("direct:ac").choice()
            .when().simple("${body.ac} == io.silverware.demos.devconf.kjar.AirConditioningCommand.Ac.NORMAL").to("direct:acOn")
            .otherwise().to("direct:acOff");
      from("direct:acOn").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/ac/on");
      from("direct:acOff").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/ac/off");

      // fireplace
      from("direct:fire").choice()
            .when().simple("${body.fire} == io.silverware.demos.devconf.kjar.FireplaceCommand.Fire.HEAT").to("direct:fireOn")
            .otherwise().to("direct:fireOff");
      from("direct:fireOn").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/fireplace/on");
      from("direct:fireOff").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/fireplace/off");

      // media center
      from("direct:media").choice()
            .when().simple("${body.media} == io.silverware.demos.devconf.kjar.MediaCenterCommand.Media.OFF").to("direct:tvOff")
            .when().simple("${body.media} == io.silverware.demos.devconf.kjar.MediaCenterCommand.Media.NEWS").to("direct:tvNews")
            .otherwise().to("direct:tvRomantic");
      from("direct:tvRomantic").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/tv/romantic");
      from("direct:tvNews").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/tv/news");
      from("direct:tvOff").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/tv/off");

      // door, aka front door, aka main entrance
      from("direct:door").choice()
            .when().simple("${body.door} == io.silverware.demos.devconf.kjar.DoorCommand.Door.FRONT").to("direct:doorFront")
      ...
      from("direct:doorOpen").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/door/open");
      from("direct:doorClose").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/door/close");
      ...
      from("direct:windowOpen").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/window/open");
      from("direct:windowClose").setHeader(Exchange.HTTP_METHOD, constant("GET")).to("jetty:http://" + iotHost + "/window/close");
